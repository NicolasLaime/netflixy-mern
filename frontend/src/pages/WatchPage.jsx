...
import { ORIGINAL_IMG_BASE_URL, SMALL_IMG_BASE_URL } from "../constats/constants";
...
const WatchPage = () => {
    ...
    useEffect(() => {
        const getSimilarTrailers = async () => {
            try {
                const res = await axios.get(`/api/v1/${contentType}/${id}/similares`);
    ...
    useEffect(() => {
        const getContentDetalles = async () => {
            try {
                const res = await axios.get(`/api/v1/${contentType}/${id}/detalles`);
    ...
    // Función para ir al trailer anterior
    const handlePrev = () => {
    ...
    // Función para ir al siguiente trailer
    const handleNext = () => {
            ...
            <div className="mx-auto container px-4 py-8 h-full">
                ...
                {trailers.length > 0 && (
                    <div className="flex justify-between items-center mb-4">
                        <button
                            className={`bg-gray-500/70 hover:bg-gray-500 text-white py-2 px-4 rounded ${currentTrailersIdx === 0 ? "cursor-not-allowed opacity-50" : ""
                                }`}
                            disabled={currentTrailersIdx === 0}
                            onClick={handlePrev} // Asegurarse de que el evento está aquí
                        >
                            <ChevronLeft size={24} />
                        </button>
                        ...
                        <button
                            ...
                            onClick={handleNext} // Mover el evento al lugar correcto
                        >
                ...
                <div className="aspect-video mb-8 p-2 sm:px-32">
                    {trailers.length > 0 && (
                        <ReactPlayer controls={true} width={'100%'} height={'70vh'} className='mx-auto overflow-hidden rounded-lg' url={`https://www.youtube.com/watch?v=${trailers[currentTrailersIdx].key}`} />
                    )}

                    {trailers?.length === 0 && (
                        <h2 className="text-xl text-center mt-5">
                            Trailer No disponible por {" "}
                ...
                {similarContent.length > 0 && (
                    <div className="mt-12 max-w-5xl mx-auto relative">
                        <h3 className="text-3xl font-bold mb-4">
                            Peliculas/Tv similares
                        </h3>

                        <div className="flex overflow-x-scroll scrollbar-hide gap-4 pb-4 group" ref={sliderRef}>
                             {similarContent.map((content) => (
                                <Link key={content.id} to={`/watch/${content.id}`} className="w-52 flex-none">
                                 <img src={SMALL_IMG_BASE_URL + content.poster_path} alt="poster-similares"  className="w-full h-auto rounded-md"/>

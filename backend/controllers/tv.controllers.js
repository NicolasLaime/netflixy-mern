...
export async function getTvDetalles(req,res) {
    const {id} = req.params
    try {
...
export async function getTvSimilares(req, res) {
    const {id} = req.params
    try {
...
export async function getTvCategoria(req, res) {
   const {categoria} = req.params
   try {
      const data = await fetchFromTMDB(`https://api.themoviedb.org/3/tv/${categoria}?language=en-ES&page=1`)
